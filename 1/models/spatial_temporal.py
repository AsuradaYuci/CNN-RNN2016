...
class Net(nn.Module):
    def __init__(self, args, nFltrs1, nFltrs2, nFltrs3, n_person_train):
        ...
        self.hid_weight = nn.Parameter(
            nn.init.xavier_uniform_(torch.Tensor(1, self.args.sampleSeqLength, self.args.embeddingSize).type(
    ...
    def build_net(self, input1, input2):
        ...
        psn1_seq1_out = seq1(input1)  # torch.Size([16, 32, 23, 19])
        psn2_seq1_out = seq1(input2)  # torch.Size([16, 32, 23, 19])
        psn1_poling = self.spatial_pooling(psn1_seq1_out)  # torch.Size([16, 2720])
        psn2_poling = self.spatial_pooling(psn2_seq1_out)  # torch.Size([16, 2720])

        psn1_seq2_out = self.seq2(psn1_poling).unsqueeze_(0)  # torch.Size([1, 16, 128])
        psn2_seq2_out = self.seq2(psn2_poling).unsqueeze_(0)  # torch.Size([1, 16, 128])

        psn1_rnn_out, hn1 = self.rnn(psn1_seq2_out, self.hid_weight)  # torch.Size([1, 16, 128])
        psn2_rnn_out, hn2 = self.rnn(psn2_seq2_out, self.hid_weight)  # torch.Size([1, 16, 128])

        psn1_rnn_out_quzed = psn1_rnn_out.squeeze()  # torch.Size([16, 128])
        psn2_rnn_out_quzed = psn2_rnn_out.squeeze()  # torch.Size([16, 128])
        feature_p, feature_g = self.atten(psn1_rnn_out_quzed, psn2_rnn_out_quzed)  # torch.Size([1, 128])
        identity_p = self.final_full_connect(feature_p)  # torch.Size([1, 192])
        identity_g = self.final_full_connect(feature_g)  # torch.Size([1, 192])
        return feature_p, feature_g, identity_p, identity_g

    def spatial_pooling(self, input):
        out1 = self.pool1(input)
        out1_shape = out1.shape  # torch.Size([16, 32, 8, 8])
        sec_dim = out1_shape[-1] * out1_shape[-2] * out1_shape[-3]
        out1 = out1.contiguous().view(-1, sec_dim)

        out2 = self.pool2(input)
        out2_shape = out2.shape  # torch.Size([16, 32, 4, 4])
        sec_dim = out2_shape[-1] * out2_shape[-2] * out2_shape[-3]
        out2 = out2.contiguous().view(-1, sec_dim)

        out3 = self.pool3(input)
        out3_shape = out3.shape  # torch.Size([16, 32, 2, 2])
        sec_dim = out3_shape[-1] * out3_shape[-2] * out3_shape[-3]
        out3 = out3.contiguous().view(-1, sec_dim)

        out4 = self.pool4(input)
        out4_shape = out4.shape  # torch.Size([16, 32, 1, 1])
        sec_dim = out4_shape[-1] * out4_shape[-2] * out4_shape[-3]
        out4 = out4.contiguous().view(-1, sec_dim)

        cat = torch.cat((out1, out2, out3, out4), 1)
        return cat  # torch.Size([16, 2720])
